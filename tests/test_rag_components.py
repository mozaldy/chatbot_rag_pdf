...
from app.services.query_router.factory import get_query_router
from app.services.query_router.models import RouteIntention, RouteResult
from app.services.query_router.llm_router import LLMQueryRouter
...
class QueryRouterTests(unittest.TestCase):
    def test_factory_returns_llm_router(self):
        router = get_query_router("llm")
        self.assertIsInstance(router, LLMQueryRouter)

    @patch("app.services.query_router.llm_router.get_fast_llm")
    def test_router_identifies_chit_chat(self, mock_get_llm):
        # Mock the LLM response using AsyncMock for the async method
        mock_llm = MagicMock()
        mock_get_llm.return_value = mock_llm
        
        # Configure achat as an AsyncMock to be awaitable
        mock_llm.achat = AsyncMock()
        mock_llm.achat.return_value = MagicMock(message=MagicMock(content='{"intention": "CHIT_CHAT", "rewritten_query": "Hello", "reasoning": "Greeting", "clarification_question": null}'))

        router = LLMQueryRouter()
        # Cleanly inject the mock (since __init__ calls get_fast_llm)
        router.llm = mock_llm 
        
        result = asyncio.run(router.route("Hello!"))
        
        self.assertEqual(result.intention, RouteIntention.CHIT_CHAT)

    @patch("app.services.query_router.llm_router.get_fast_llm")
    def test_router_handles_search(self, mock_get_llm):
        mock_llm = MagicMock()
        mock_get_llm.return_value = mock_llm
        
        mock_llm.achat = AsyncMock()
        mock_llm.achat.return_value = MagicMock(message=MagicMock(content='{"intention": "SEARCH", "rewritten_query": "sales data last month", "reasoning": "User asked for specific data", "clarification_question": null}'))

        router = LLMQueryRouter()
        router.llm = mock_llm
        
        result = asyncio.run(router.route("Find sales data for last month"))
        
        self.assertEqual(result.intention, RouteIntention.SEARCH)
        self.assertEqual(result.rewritten_query, "sales data last month")

    @patch("app.services.query_router.llm_router.get_fast_llm")
    def test_router_handles_json_markdown_block(self, mock_get_llm):
        """Test that the router can clean up ```json ... ``` blocks."""
        mock_llm = MagicMock()
        mock_get_llm.return_value = mock_llm
        
        # LLM returns markdown fenced JSON
        json_content = '```json\n{"intention": "SUMMARIZATION", "rewritten_query": "summary", "reasoning": "summary", "clarification_question": null}\n```'
        mock_llm.achat = AsyncMock()
        mock_llm.achat.return_value = MagicMock(message=MagicMock(content=json_content))

        router = LLMQueryRouter()
        router.llm = mock_llm
        
        result = asyncio.run(router.route("Summarize this."))
        
        self.assertEqual(result.intention, RouteIntention.SUMMARIZATION)
